...
function printUsage() {
  console.log('Uso: npm run eas:run -- <ios|android|all> <preview|build> [perfil] [args extras]');
  console.log('');
  ...
  console.log('');
  console.log('Perfis (apenas para build, padrão: production):');
  console.log('  production  Build com perfil de produção');
  console.log('  preview     Build com perfil de preview');
  console.log('');
...
if (!['preview', 'build'].includes(mode)) {
  console.error(`Modo inválido: "${modeArg}"`);
  printUsage();
  process.exit(1);
}

const explicitProfile = mode === 'build' && ['preview', 'production'].includes(profileArg);
...
function runEas(platformToRun) {
  const easArgs =
    mode === 'preview'
      ? ['eas', 'update', '--platform', platformToRun, '--channel', 'preview', ...extraArgs]
      : ['eas', 'build', '--platform', platformToRun, '--profile', profile, ...extraArgs];
